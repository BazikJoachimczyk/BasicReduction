...
import numpy as np
from utils import FitsFilesData, CutSubframe
from masters import Masters

masterFrames = Masters()  

def CreateMasterFrames(path, subframeParams = None):
    """
    ...
    for i in range(len(bias_subpath)):
        ...
        if (subframeParams != None):
            mbias_frame.data = CutSubframe(mbias_frame.data, subframeParams)
    ...
    for i in range(len(dark_subpath)):
        ...
        if (subframeParams != None):
            med_data_d = CutSubframe(med_data_d, subframeParams)                
    ...
    for i in range(len(flat_subpath)):
        ...
        data_f = FitsFilesData(join(flat_path, flat_subpath[i]), subframeParams)        
